...
class SupervisorandSubordinateReports extends Component {
  constructor(props) {
    super(props);
...
alert("DATA TO BACKEND"+JSON.stringify({
  ...
  totlaItemCount: this.state.totalItemCount,
    ...
    if (this.state.fromDate != "" && this.state.toDate != "" && this.state.supervisor != "" ) {
      ...
      //   url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/EmployeeReportPaging/supervisorbasedReport",
        //url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/EmployeeReportPaging/PeriodWiseShiftHistoryReport",
       url: "http://15.206.129.105:8080/MerchandiseAPI/EmployeeReportPaging/supervisorbasedReport",
  ...
  DownloadExcel() {
    ...
    if (xlsRows.length != 0) {
      ...
      if (self.state.fromDate != "" && self.state.toDate != "" && self.state.supervisor != " "  &&self.state.downloadData == "yes") {
  ...
  GenerateExcel() {
    ...
    /* XLS Head Columns */
    var xlsHeaderMain = ["SUPERVISOR REPORT LIST" + " [ " + this.state.fromDate + " - " + this.state.toDate + " -"+this.state.supervisor+"]"];
    ...
    /* File Name */
    var filename = "supervisorReport.xlsx";

    /* Sheet Name */
    var ws_name = "supervisorReport";
  ...
  GeneratePDF() {
    ...
    doc.setFontSize(11);
    doc.text(" SupervisorReport" + " [ " + this.state.fromDate + " - " + this.state.toDate + "-"+this.state.supervisor+" ]", 10, 10);
    var indexValue = 0;
    var pdfData = [];
    var listInnerRowData = [];
    ...
    /* File Name */
    var filename =  "SupervisorReport" + this.state.supervisor + ".pdf";
    doc.autoTable({
...
var companyId="001";
    ...
    $.ajax({
      ...
      //url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/ShiftReport/selectTotalNoShift",
       // url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/ShiftReport/selectSupervisorDetails",
     url: "http://15.206.129.105:8080/MerchandiseAPI/EmployeeReportPaging/selectSupervisorDetails",
      contentType: "application/json",
      dataType: 'json',
      async: false,
      success: function (data, textStatus, jqXHR) {
        ...
        if (data.length != 0) {
          var supervisorDetailsArray = [];
          $.each(data, function (i, item) {
            supervisorDetailsArray.push({ label: item.shiftSuperVisorId + "-" + item.shiftSupervisorName , value: item.shiftSuperVisorId });
          });
          supervisorDetailsArray.push({ label: "All", value: 'all' });

          self.state.supervisorDetailsOptions = supervisorDetailsArray;
          self.setState({
            supervisorDetailsOptions: self.state.supervisorDetailsOptions,
          })
        }
  ...
  handleChangeSupervisorIdOption = (e) => {

    // Display selected value for user
    var currentValue = e;
    this.state.supervisorOption = e;
    this.state.supervisor = e.value;

    this.setState({
      supervisorOption: this.state.supervisorOption,
      supervisor: this.state.supervisor,
  ...
  //  var companyId = CryptoJS.AES.decrypt(localStorage.getItem('CompanyId'), "shinchanbaby").toString(CryptoJS.enc.Utf8)
   ...
   var companyId = '001';
    ...
    $.ajax({
     ...
     // url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/excelDownload/supervisorbasedReport",
      url: "http://15.206.129.105:8080/MerchandiseAPI/excelDownload/supervisorbasedReport",
      ...
      <div className="container">
        ...
        <div class="btn-group" style={{ marginBottom: "0%", width: "100%" }}>
          ...
          <form >
            <label
              htmlFor="toDate"
              style={{ marginRight: '50%' }}> Supervisor:</label>
          <Select
                  id="supervisor"
                  style={{ width: '50%' }}
                  name="supervisor"
                  options={this.state.supervisorDetailsOptions}
                  onChange={this.handleChangeSupervisorIdOption}
                  value={this.state.supervisorOption}
                />
                  </form>
        </div>
...
export default SupervisorandSubordinateReports;
