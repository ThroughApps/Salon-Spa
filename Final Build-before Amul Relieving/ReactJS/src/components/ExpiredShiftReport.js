...
class ExpiredShiftReport extends Component {
  constructor(props) {
    super(props);
 ...
 var fromDate=dateAdd.getFullYear()+'-'+("0" + (dateAdd.getMonth() + 1)).slice(-2)+'-'+("0" + (dateAdd.getDate())).slice(-2);
   ...
   // var employeeId = CryptoJS.AES.decrypt(localStorage.getItem("EmployeeId"), "shinchanbaby").toString(CryptoJS.enc.Utf8);
    ...
    if (this.state.fromDate != "" && this.state.toDate != "") {
      ...
      $.ajax({
        ...
        //url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/EmployeeReportPaging/PeriodWiseShiftHistoryReport",
        url: "http://15.206.129.105:8080/MerchandiseAPI/EmployeeReportPaging/expiredShiftReport",
        ...
        success: function (data, textStatus, jqXHR) {
          window.scrollTo(0, 0);
          console.log("DATA FROM BACKEND:", data.expiredShiftReportList);
          ...
          if (data.expiredShiftReportList.length != 0) {
            ...
            localPageData[self.state.activePage].push(data.expiredShiftReportList);

            var datavalue = 0;

            $.each(data.expiredShiftReportList, function (i, itemk) {
  ...
  GenerateExcel() {
    ...
    /* XLS Head Columns */
    var xlsHeaderMain = ["EXPIRED SHIFT REPORT LIST" + " [ " + this.state.fromDate + " - " + this.state.toDate + " ]"];
    createXLSLFormatObj.push(xlsHeaderMain);


    /* XLS Head Columns */
    var xlsHeader = ["S.NO", "SHIFT","DESCRIPTION", "START DATE", "END DATE"];
    ...
    /* File Name */
    var filename = "expiredShiftReport.xlsx";

    /* Sheet Name */
    var ws_name = "expiredShiftReport";
  ...
  GeneratePDF() {
    ...
    doc.setFontSize(11);
    doc.text("EXPIRED SHIFT REPORT LIST" + " [ " + this.state.fromDate + " - " + this.state.toDate + " ]", 10, 10);
    var indexValue = 0;
    var pdfData = [];
    var listInnerRowData = [];
    ...
    /* File Name */
    var filename = "expiredShiftReport.pdf";
    doc.autoTable({
      head: [["S.NO", "Shift","DESCRIPTION", "StartDate", "EndDate",]],
      body: pdfData,
    }
    );
...
count=count+1;
  ...
  var companyId = '001';
    ...
    $.ajax({
      ...
      //url: "http://15.206.129.105:8080/EmployeeAttendenceAPI/excelDownload/PeriodWiseShiftWiseReport",
      url: "http://15.206.129.105:8080/MerchandiseAPI/excelDownload/expiredShiftReport",
      ...
      success: function (data, textStatus, jqXHR) {
        ...
        if (data.expiredShiftReportListExcel.length != 0) {

       



          $.each(data.expiredShiftReportListExcel, function (i, itemk) {
  ...
  render() {
    return (
      <div className="container">
        ...
        <h4 style={{ textAlign: "center" }}>Expired Shift Report - {this.state.fromDate} - {this.state.toDate}</h4>
...
export default ExpiredShiftReport;
