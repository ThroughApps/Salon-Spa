...
self.setState({
  data:self.state.data,
    ...
    $.ajax({
      ...
      url: " http://15.206.129.105:8080/MerchandiseAPI/gst/businesstobusinessreport",
      contentType: "application/json",
      dataType: 'json',
      async: false,
      success: function (data, textStatus, jqXHR) {
      var tab = '<thead><tr class="headcolor"><th>Invoice No</th><th>Invoice Date</th>'
      +'<th>Vendor Name</th><th>Taxable Value</th><th>CGST</th><th>SGST</th><th>IGST</th><th>Total</th></tr></thead>';
        $.each(data.businesstobusinessreportlist, function (i, item) {
 
          tab += '<tbody id= "myTable" ><tr id="tabletextcol" ><td>' + item.invoiceNo + '</td>'
          +'<td>' + item.invoiceDate + '</td><td>' + item.vendorName + '</td>'
          +'<td>' + item.totalgst + '</td><td>' + item.totalcgst + '</td><td>' + item.totalsgst + '</td><td>' + item.totaligst + '</td><td>' + item.subtotal1 + '</td></tr></tbody>';
...
BackbtnFunc() {
  ...
  render() {
    ...
    <li class="active"><a  style={{color:"black",fontWeight:"bold"}}  className="active"  onClick={() => this.B2BFunc()}><span style={{display:"inline-grid"}}>Business to Business</span></a></li>
    <li><a  style={{color:"black",fontWeight:"bold"}}   onClick={() => this.B2CFunc()}><span style={{display:"inline-grid"}}>Business to Customer</span></a></li>
    </ul>
